...
<div class="center1020">
	...
	<sql:query var="result" dataSource="${dbInfo.connectionPool.dataSource}">
		...
		WHEN gr = 824633720832
			THEN 'Палатка (Магтел - частные лица), Дебетовые абонплаты (изменяет статус дебетовых договоров в случае при балансе <=0)'	
		WHEN gr = 70368744177664
			THEN 'Дебин - физические лица'	
		WHEN gr = 524288
			THEN 'Удостоверяющий Центр'		
		WHEN gr = 64
			THEN 'Ягодное (Магтел - частные лица)'
		ELSE 'Другие группы'  
	    END AS "Группа"
	    FROM contract, (SELECT @row_number:=0) AS t  
	    WHERE date1=SUBDATE(CURDATE(), 0) AND gr & 1081004648040234863 > 0 AND del=0 
	    ORDER BY title 
