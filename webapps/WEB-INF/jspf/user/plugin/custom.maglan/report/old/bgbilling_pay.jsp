...
<div class="center1020">
	...
	<sql:query var="result" dataSource="${dbInfo.connectionPool.dataSource}">
	    ...
	    SELECT t1.dt AS "Дата:", t1.summa AS "Платеж:",
		    CASE
			WHEN pt = 2
		    ...
		    ELSE 'Другие кассы'
		    END AS "КАССА:",
		t1.comment AS "Комментарий:", t2.title AS "Номер договора:", t2.comment AS "Название договора:", t1.lm AS "Время транзакции:", (SELECT sum(t1.summa) 
		FROM contract_payment AS t1, contract AS t2 
		WHERE t1.cid=t2.id AND dt = CURDATE() and t1.pt in (2, 24)) AS "ИТОГО:"
		FROM contract_payment AS t1, contract AS t2 
		WHERE t1.cid=t2.id AND dt = CURDATE() and t1.pt in (2, 24) order by t1.lm asc
