...
<div class="center1020">
	...
	SELECT t1.dt AS "Дата:", FORMAT ((t1.summa),2) AS "Платеж:",
	    ...
	    (SELECT FORMAT ((sum(t1.summa)),2) FROM contract_payment AS t1, contract AS t2 WHERE t1.cid=t2.id AND dt >= subdate(curdate(), (day(curdate())-1)) and t1.pt in (2, 24)) AS "ИТОГО:"
	    FROM contract_payment AS t1, contract AS t2
	    WHERE t1.cid=t2.id AND dt >= subdate(curdate(), (day(curdate())-1)) AND t1.pt in (2, 24) order by t1.lm asc
